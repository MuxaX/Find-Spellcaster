...
// alternative fire weapon
event EAltFireWeapon {};
// release alt weapon
event EReleaseWeaponAlt  {};
...
294 BOOL m_bAltFire    = FALSE,       // weapon is alternative firing
...
284 INDEX m_iBulletsMagazin     = 0,
 41 INDEX m_iMaxBullets     = MAX_BULLETS,
 42 INDEX m_iShells         = 0,
291 INDEX m_iShellsMagazin  = 0,
...
285 INDEX m_iPistol         = 0,
286 INDEX m_iPistolMagazin  = 0,
287 INDEX m_iMaxPistol      = MAX_PISTOL,
288 INDEX m_iPistolOnFireStart = 0,
289 INDEX m_iPistolOnCur = 0,
// double colt
309 INDEX m_iSMG            = 0,
310 INDEX m_iSMGMagazin     = 0,
311 INDEX m_iMaxSMG         = MAX_SMG,
312 INDEX m_iSMGOnFireStart = 0,
313 INDEX m_iSMGOnCur       = 0,
// minigun
...
components:
  ...
  9 class   CLASS_ALT_CANNONBALL    "Classes\\AltCannonBall.ecl",
...
// ************** COLT **************
 ...
 300 model  MODEL_P90                   "Models\\Weapons\\P90\\P90.mdl",
 301 model  MODEL_P90BODY               "Models\\Weapons\\P90\\p90_body.mdl",
 302 model  MODEL_P90MAGAZIN            "Models\\Weapons\\P90\\p90_reloadM.mdl",
 303 model  MODEL_P90ZATVOR             "Models\\Weapons\\P90\\p90_zatvorM.mdl",
 304 texture TEXTURE_P90                "Models\\Weapons\\P90\\p90.tex",
 305 sound   SOUND_P90_FIRE             "Models\\Weapons\\P90\\Sounds\\fire.wav",
 306 sound   SOUND_P90_RELOAD           "Models\\Weapons\\P90\\Sounds\\reload.wav",
 307 sound   SOUND_P90_ACTIVATE         "Models\\Weapons\\P90\\Sounds\\_activate.wav",
...
290 sound   SOUND_SINGLESHOTGUN_RELOAD  "Models\\Weapons\\SingleShotgun\\Sounds\\Reload.wav",
...
// ************** TOMMYGUN **************
 ...
 75 model   MODEL_UNDERBARREL           "Models\\Weapons\\TommyGun\\af36UndetBarrel.mdl",
 76 model   MODEL_AGGRANAT              "Models\\Weapons\\TommyGun\\ag36GRANAT.mdl",
281 sound   SOUND_TOMMYGUN_RELOAD       "Models\\Weapons\\TommyGun\\Sounds\\_Reolad.wav",
282 sound   SOUND_TOMMYGUN_ACTIVATE     "Models\\Weapons\\TommyGun\\Sounds\\_Activate.wav",
...
// ************** ROCKET LAUNCHER **************
 ...
 309 sound   SOUND_ROCKETLAUNCHER_ACTIVATE   "Models\\Weapons\\RocketLauncher\\Sounds\\_activate.wav",
...
114 model   MODEL_SNIPER_SHUTTER        "ModelsMP\\Weapons\\Sniper\\SniperShutter.mdl",
...
310 sound   SOUND_SNIPER_ACTIVATE           "ModelsMP\\Weapons\\Sniper\\Sounds\\_activate.wav",
...
//    plan.m_fRecoilSpeed += wpn_fRecoilSpeed[m_iCurrentWeapon];
                          ...
                          BOOL bRender, INDEX iEye)
  {
    ...
    if (tmSeriousDamage>tmNow && tmInvulnerability>tmNow) {
      ...
      Particles_ModelGlow2(&m_moWeapon, plWeapon, tmInvulnerability, PT_STAR05, 0.025f, /*tmp_ai[1]*/2, 0.01f, 0x57a63900);
      Particle_EndSystem();
  ...
  void SetCurrentWeaponModel(void) {
    ...
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_DOUBLECOLT: {
        ...
        //m_moWeapon.PlayAnim(P90_ANIM_P90_ANIM_WAIT3, 0);
		break; }
      case WEAPON_SINGLESHOTGUN: {
      ...
      case WEAPON_MINIGUN: {
        ...
        //AddAttachmentToModel(this, m_moWeapon, MINIGUN_ATTACHMENT_ENGINE, MODEL_MG_ENGINE, TEXTURE_MG_BODY, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
...
//    }
  ...
  void FireAltCannonRay()
  {
    ...
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.fLaunchPower = 60.0f+3*4.0f; // ranges from 60-140 (since iPower can be max 20)
    eLaunch.fSize = 3.0f;
    penBall->Initialize(eLaunch);
  };
...
//    if( iTakeAmmo & (1<<AMMO_NUKEBALLS))     {m_iNukeBalls  = 0;}
  ...
  INDEX GetAmmo(void)
  {
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_TOMMYGUN:        return m_iBullets /*&& m_iGrenades*/;
  ...
  INDEX GetMaxAmmo(void)
  {
    switch (m_iCurrentWeapon) {
      ...
      case WEAPON_TOMMYGUN:        return m_iMaxBullets /*&& m_iMaxGrenades*/;
...
/*  } else if (m_iCurrentWeapon==WEAPON_GHOSTBUSTER) {
      ...
      wait() {
        on (EBegin) : {
          ...
          switch (m_iCurrentWeapon) {
			...
			case WEAPON_MINIGUN: call MiniGunFire(); break;
			...
			case WEAPON_IRONCANNON: call AltCannonFire(); break;
            default: ASSERTALWAYS("Unknown weapon.");
          }
          resume;
        }
    ...
    switch (m_iCurrentWeapon) {
	  ...
	  case WEAPON_DOUBLECOLT: { jump DoubleColtStop(); break; }
...
//      case WEAPON_GHOSTBUSTER: { jump GhostBusterStop(); break; }
  ...
  AltFire(){
   ...
   PlaySound(pl.m_soWeapon0, SOUND_SILENCE, SOF_3D|SOF_VOLUMETRIC);      // stop possible sounds
	...
	if ((m_iCurrentWeapon==WEAPON_IRONCANNON) /*|| (m_iCurrentWeapon==WEAPON_NUKECANNON)*/) {
      jump CannonFireStart();
    }

	while (HoldingAltFire() && m_bHasAmmo) {
      // boring animation
      ((CPlayerAnimator&)*((CPlayer&)*m_penPlayer).m_penAnimator).m_fLastActionTime = _pTimer->CurrentTick();
      wait() {
        on (EBegin) : {
		  ...
		  case WEAPON_KNIFE: call SwingKnife(); break;
            ...
            case WEAPON_SINGLESHOTGUN: call FireSingleShotgun(); break;
            case WEAPON_DOUBLESHOTGUN: call FireDoubleShotgun(); break;
            case WEAPON_TOMMYGUN: call AltFireTommyGun(); break;
			case WEAPON_MINIGUN: call MiniGunFire(); break;
            case WEAPON_SNIPER: call FireSniper(); break;
            case WEAPON_ROCKETLAUNCHER: call FireRocketLauncher(); break;
            case WEAPON_GRENADELAUNCHER: call FireGrenadeLauncher(); break;
            case WEAPON_LASER: call FireLaser(); break;
			case WEAPON_IRONCANNON: call CannonFireStart(); break;
  ...
  if( pl.m_pstState!=PST_DIVE && hud_bShowWeapon)
  {
    ...
    pl.m_soWeapon0.Set3DParameters(500.0f, 5.0f, 0.0f, 1.0f);      // mute fire
  ...
  if (m_iSMGMagazin>0){
	 ...
	 if( hud_bShowWeapon)
	  ...
	  autowait(m_moWeapon.GetAnimLength(iAnim)-0.05f);
	  // no ammo -> change weapon
	  if (m_iSMG<=0 && m_iSMGMagazin<=0) { SelectNewWeapon(); }
  }else{
      ASSERTALWAYS("Desert eagle - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
  }
  ...
  if( pl.m_pstState!=PST_DIVE && hud_bShowWeapon)
  {
      ...
      // no ammo -> change weapon
      if (m_iShells<=0 && m_iShellsMagazin<=0) { SelectNewWeapon(); }
    } else {
      ASSERTALWAYS("Pancor - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
    }
	// no more shells in Pancor -> reload
	   if (m_iShellsMagazin == 0){
	   jump ReloadSingleShotgun();
	   }
    return EEnd();
...
*/
  ...
  FireTommyGun() {
	...
	/*else if (m_iBulletsMagazin < 50 && m_iBulletsMagazin > 0) {
	  m_bReloadWeapon = FALSE;
	  autocall ReloadTommyGunBullets() EEnd;
	  jump Idle();
	}*/
	//CPrintF("m_iBulletsMagazin = %i\n", m_iBulletsMagazin);
	//CPrintF("m_iBullets = %i\n", m_iBullets);
  ...
  AltFireTommyGun(){
    ...
    if (m_iGrenades>0) {
      ...
      autowait(m_moWeapon.GetAnimLength(TOMMYGUN_ANIM_RELOADG)-0.05f);
  ...
  return EEnd();
    ...
    if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("TommyGun_reload");}
    autowait(m_moWeapon.GetAnimLength(TOMMYGUN_ANIM_RELOAD));
	} else if(m_iBulletsMagazin < 50 && m_iBulletsMagazin != 0 && m_iBullets < 50){
	m_iBullets -= (50 - m_iBulletsMagazin);
    m_iBulletsMagazin = m_iBullets - (50 - m_iBullets);
	//sound
	CPlayer &pl = (CPlayer&)*m_penPlayer;
    PlaySound(pl.m_soWeapon1, SOUND_TOMMYGUN_RELOAD, SOF_3D|SOF_VOLUMETRIC);
...
//    PlaySound(pl.m_soWeapon0, SOUND_SILENCE, SOF_3D|SOF_VOLUMETRIC/*|SOF_SMOOTHCHANGE*/);
  ...
  */
  ...
  CannonFireStart()
  {
    ...
    while (HoldingAltFire() && ((_pTimer->CurrentTick()-TM_START)<1.0f) )
    {
      autowait(_pTimer->TickQuantum);
      INDEX iPower = INDEX((_pTimer->CurrentTick()-TM_START)/_pTimer->TickQuantum);
  ...
  if (m_iIronBalls>0) {
      ...
      autowait(m_moWeapon.GetAnimLength(CANNON_ANIM_FIRE)-0.05f);


      // no ammo -> change weapon
      if (m_iIronBalls<=0) { SelectNewWeapon(); }
    } else {
      ASSERTALWAYS("Cannon - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
    }
