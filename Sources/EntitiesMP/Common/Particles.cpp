...
static CTextureObject _toLaserBlueTrail;
...
void InitParticles(void)
{
  try
  {
	...
	_toLaserBlueTrail.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\CircleRail.tex"));
	...
	_toIcek.SetData_t(CTFILENAME("Textures\\Effects\\Particles\\Icek.tex"));
...
void CloseParticles(void)
{
  ...
  _toLaserBlueTrail.SetData(NULL);
...
void Particles_SpiralTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(SPIRAL_TRAIL_POSITIONS);
}
...
#define LASERBLUE_TRAIL_POSITIONS 40
#define LASERBLUE_TRAIL_INTERPOSITIONS 1
void Particles_LaserBlueTrail_Prepare(CEntity *pen)
{
  pen->GetLastPositions(LASERBLUE_TRAIL_POSITIONS);
}
void Particles_LaserBlueTrail(CEntity *pen, FLOAT fStretch)
{
  CLastPositions *plp = pen->GetLastPositions(LASERBLUE_TRAIL_POSITIONS);
  FLOAT fSeconds = _pTimer->GetLerpedCurrentTick();
  
  Particle_PrepareTexture(&_toLaserBlueTrail, PBT_ADD);
  Particle_SetTexturePart( 512, 512, 0, 0);

  const FLOAT3D *pvPos1;
  const FLOAT3D *pvPos2 = &plp->GetPosition(1);
  INDEX iParticle = 0;
  INDEX iParticlesLiving = plp->lp_ctUsed*LASERBLUE_TRAIL_INTERPOSITIONS;
  for( INDEX iPos=2; iPos<plp->lp_ctUsed; iPos++)
  {
    pvPos1 = pvPos2;
    pvPos2 = &plp->GetPosition(iPos);
    ...
    for (INDEX iInter=0; iInter<LASERBLUE_TRAIL_INTERPOSITIONS; iInter++)
    {
      FLOAT fRand = rand()/FLOAT(RAND_MAX);
      FLOAT fAngle = 0.0f;
      FLOAT3D vPos = Lerp(*pvPos1, *pvPos2, iInter*1.0f/LASERBLUE_TRAIL_INTERPOSITIONS);
      FLOAT fSize = iParticle*0.5f/iParticlesLiving*fStretch+0.25f;

      UBYTE ub = 255-(UBYTE)((ULONG)iParticle*255/iParticlesLiving);
      FLOAT fLerpFactor = FLOAT(iPos)/LASERBLUE_TRAIL_POSITIONS;
      COLOR colColor = LerpColor( C_WHITE, C_BLACK, fLerpFactor);
      Particle_RenderSquare( vPos, fSize, fAngle, colColor|ub);
      iParticle++;
    }
  ...
  for( iPos=2; iPos<plp->lp_ctUsed; iPos++)
  {
    ...
    UBYTE ub = UBYTE(255-iPos*256/plp->lp_ctUsed);
    FLOAT fSize = iPos*0.01f*fStretch+0.005f;
    Particle_RenderLine( vPos, vOldPos, fSize, RGBToColor(5&ub,77&ub,89&ub)|ub);
    vOldPos=vPos;
  }
  // all done
  Particle_Flush();
