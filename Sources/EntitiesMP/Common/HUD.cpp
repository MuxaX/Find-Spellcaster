...
// status bar textures
//change health icon 
static CTextureObject _toSmallHealth;
static CTextureObject _toMiddleHealth;
...
static CTextureObject _toAllBulletsAll;

//draw bar textures
static CTextureObject _tohealthBar1;
static CTextureObject _tohealthBar2;
static CTextureObject _toArmorBar;
static CTextureObject _toOxygenBar;

// ammo textures     
static CTextureObject _toAPistol;  
static CTextureObject _toASMG;     
...
static struct AmmoInfo _aaiAmmo[10] = {
  { &_toAPistol,        &_awiWeapons[2],  NULL,             0, 0, 0, -10, FALSE }, //  0
  { &_toASMG,           &_awiWeapons[3],  NULL,             0, 0, 0, -10, FALSE }, //  0
  { &_toAShells,        &_awiWeapons[4],  &_awiWeapons[5],  0, 0, 0, -10, FALSE }, //  1
  { &_toABullets,       &_awiWeapons[6],  &_awiWeapons[7],  0, 0, 0, -10, FALSE }, //  2
  { &_toARockets,       &_awiWeapons[8],  NULL,             0, 0, 0, -10, FALSE }, //  3
  { &_toAGrenades,      &_awiWeapons[9],  NULL,             0, 0, 0, -10, FALSE }, //  4
  { &_toANapalm,        &_awiWeapons[11], NULL,             0, 0, 0, -10, FALSE }, //  5
  { &_toAElectricity,   &_awiWeapons[12], NULL,             0, 0, 0, -10, FALSE }, //  6
  { &_toAIronBall,      &_awiWeapons[14], NULL,             0, 0, 0, -10, FALSE }, //  7
  { &_toASniperBullets, &_awiWeapons[13], NULL,             0, 0, 0, -10, FALSE }, //  8
};
static const INDEX aiAmmoRemap[10] = { 0, 1, 2, 3, 4, 7, 5, 6, 8, 9 };

struct WeaponInfo _awiWeapons[18] = {
  { WEAPON_NONE,            NULL,                 NULL,         FALSE },   //  0
  { WEAPON_KNIFE,           &_toWKnife,           NULL,         FALSE },   //  1
  { WEAPON_COLT,            &_toWColt,            &_aaiAmmo[0], FALSE },   //  2
  { WEAPON_DOUBLECOLT,      &_toWDoubleColt,      &_aaiAmmo[1], FALSE },   //  3
  { WEAPON_SINGLESHOTGUN,   &_toWSingleShotgun,   &_aaiAmmo[2], FALSE },   //  4
  { WEAPON_DOUBLESHOTGUN,   &_toWDoubleShotgun,   &_aaiAmmo[2], FALSE },   //  5
  { WEAPON_TOMMYGUN,        &_toWTommygun,        &_aaiAmmo[3], FALSE },   //  6
  { WEAPON_MINIGUN,         &_toWMinigun,         &_aaiAmmo[3], FALSE },   //  7
  { WEAPON_ROCKETLAUNCHER,  &_toWRocketLauncher,  &_aaiAmmo[4], FALSE },   //  8
  { WEAPON_GRENADELAUNCHER, &_toWGrenadeLauncher, &_aaiAmmo[5], FALSE },   //  9
  { WEAPON_CHAINSAW,        &_toWChainsaw,        NULL,         FALSE },   // 10
  { WEAPON_FLAMER,          &_toWFlamer,          &_aaiAmmo[6], FALSE },   // 11
  { WEAPON_LASER,           &_toWLaser,           &_aaiAmmo[7], FALSE },   // 12
  { WEAPON_SNIPER,          &_toWSniper,          &_aaiAmmo[9], FALSE },   // 13
  { WEAPON_IRONCANNON,      &_toWIronCannon,      &_aaiAmmo[8], FALSE },   // 14
...
static void HUD_DrawSniperMask( void )
{
  ...
  colMask = LerpColor(SE_COL_BLUE, C_WHITE, 0.25f);
...
static void HUD_DrawSniperMaskAG( void )
{
  // determine location
  const FLOAT fSizeI = _pixDPWidth;
  const FLOAT fSizeJ = _pixDPHeight;
  ...
  // main sniper mask
  _pDP->InitTexture( &_toSniperMaskAG);
  ...
  colMask = LerpColor(COL_OR_RED, C_WHITE, 0.25f);
 ...
 //   fCenterJ+36.0f*_fYResolutionScaling, 15.0f*_fYResolutionScaling, colLED);
  ...
  if (_fResolutionScaling>=1.0f)
  {
    ...
    // arrow + distance
    DrawAspectCorrectTextureCentered(&_toSniperArrow, fCenterI-_fLeftX*_fYResolutionScaling,
      fCenterJ-_fLeftYU*_fYResolutionScaling, _fIconSize*_fYResolutionScaling, COL_OR_RED );
    if (fDistance>9999.9f) { strTmp.PrintF("---.-");           }
    else if (TRUE)         { strTmp.PrintF("%.1fm", fDistance); }
    _pDP->PutTextC( strTmp, fCenterI-_fLeftX*_fYResolutionScaling,
      (fCenterJ+_fLeftYD*_fYResolutionScaling)+10.0f, COL_OR_RED|CT_OPAQUE);
    
    // eye + zoom level
    DrawAspectCorrectTextureCentered(&_toSniperEye,   fCenterI+_fRightX*_fYResolutionScaling,
      fCenterJ-_fRightYU*_fYResolutionScaling, _fIconSize*_fYResolutionScaling, COL_OR_RED ); //SE_COL_ORANGE_L
    strTmp.PrintF("%.1fx", fZoom);
    _pDP->PutTextC( strTmp, fCenterI+_fRightX*_fYResolutionScaling,
      (fCenterJ+_fRightYD*_fYResolutionScaling)+10.0f, COL_OR_RED|CT_OPAQUE);
  }
}

static void HUD_DrawSniperMaskRL( void )
{
  // determine location
  const FLOAT fSizeI = _pixDPWidth;
  const FLOAT fSizeJ = _pixDPHeight;
  ...
  // main sniper mask
  _pDP->InitTexture( &_toSniperMaskRL);
  //_pDP->AddTexture( fBlackStrip, 0, fCenterI, fSizeJ, 1.00f, 0.00f, 0, 1.0f, colMask);
  //_pDP->AddTexture( fCenterI, 0, fSizeI-fBlackStrip, fSizeJ, 0, 0.00f, 1.00f, 1.0f, colMask);
  _pDP->AddTexture( fBlackStrip, 0, fCenterI, fSizeJ, 0, 0.00f, 1.00f, 1.0f, colMask);
  _pDP->AddTexture( fCenterI, 0, fSizeI-fBlackStrip, fSizeJ, 1.00f, 0.00f, 0, 1.0f, colMask);
  //_pDP->AddTexture( fBlackStrip, fCenterJ, fCenterI, fSizeJ, 0.98f, 1.0f, 0, 0.02f, colMask);
  //_pDP->AddTexture( fCenterI, fCenterJ, fSizeI-fBlackStrip, fSizeJ, 0, 1, 0.98f, 0.02f, colMask);
  _pDP->FlushRenderingQueue();
  _pDP->Fill( 0, 0, fBlackStrip+1, fSizeJ, C_BLACK|CT_OPAQUE);
...
{
  // ammo quantities
  _aaiAmmo[0].ai_iAmmoAmmount    = _penWeapons->m_iPistol;
  _aaiAmmo[0].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxPistol;
  _aaiAmmo[0].ai_iMagazinAmmo    = _penWeapons->m_iPistolMagazin;
  _aaiAmmo[1].ai_iAmmoAmmount    = _penWeapons->m_iSMG;
  _aaiAmmo[1].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxSMG;
  _aaiAmmo[1].ai_iMagazinAmmo    = _penWeapons->m_iSMGMagazin;
  _aaiAmmo[2].ai_iAmmoAmmount    = _penWeapons->m_iShells;
  _aaiAmmo[2].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxShells;
  _aaiAmmo[2].ai_iMagazinAmmo    = _penWeapons->m_iShellsMagazin;
  _aaiAmmo[3].ai_iAmmoAmmount    = _penWeapons->m_iBullets;
  _aaiAmmo[3].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxBullets;
  _aaiAmmo[3].ai_iMagazinAmmo    = _penWeapons->m_iBulletsMagazin;
  _aaiAmmo[4].ai_iAmmoAmmount    = _penWeapons->m_iRockets;
  _aaiAmmo[4].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxRockets;
  _aaiAmmo[5].ai_iAmmoAmmount    = _penWeapons->m_iGrenades;
  _aaiAmmo[5].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxGrenades;
  _aaiAmmo[6].ai_iAmmoAmmount    = _penWeapons->m_iNapalm;
  _aaiAmmo[6].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxNapalm;
  _aaiAmmo[7].ai_iAmmoAmmount    = _penWeapons->m_iElectricity;
  _aaiAmmo[7].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxElectricity;
  _aaiAmmo[8].ai_iAmmoAmmount    = _penWeapons->m_iIronBalls;
  _aaiAmmo[8].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxIronBalls;
  _aaiAmmo[9].ai_iAmmoAmmount    = _penWeapons->m_iSniperBullets;
  _aaiAmmo[9].ai_iMaxAmmoAmmount = _penWeapons->m_iMaxSniperBullets;
...
extern void DrawHUD( const CPlayer *penPlayerCurrent, CDrawPort *pdpCurrent, BOOL bSnooping, const CPlayer *penPlayerOwner)
{
  ...
  COLOR colMax = COL_GREEN;
  COLOR colTop = COL_YELLOW;
  ...
  if ((((CPlayerWeapons*)&*penPlayerOwner->m_penWeapons)->m_iCurrentWeapon==WEAPON_TOMMYGUN
    &&((CPlayerWeapons*)&*penPlayerOwner->m_penWeapons)->m_bSniping) ) {
    HUD_DrawSniperMaskAG();
  }
...
_pDP->FlushRenderingQueue();
  ...
  if( ptoCurrentAmmo!=NULL && !GetSP()->sp_bInfiniteAmmo) {
	...
	FLOAT fMagazinValue = _penWeapons->GetMagazinAmmo();
    fNormValue = fValue / fMaxValue;
    strValue.PrintF( "%d", (SLONG)ceil(fValue));
    PrepareColorTransitions( C_WHITE, C_WHITE, C_WHITE, C_RED, 0.30f, 0.15f, FALSE);
    ...
    HUD_DrawBorder( fCol+fMoverX, fRow+fMoverY, fOneUnit*0.5f, fOneUnit*0.5f, colBorder);
    fCol += fAdvUnit+fChrUnit*3/2 -fHalfUnit+280;
    HUD_DrawBorder( fCol, fRow, fChrUnit*3, fOneUnit, colBorder);
    if( bDrawAmmoIcon) {
      fCol += fAdvUnit+fChrUnit*3/2 -fHalfUnit;
      HUD_DrawBorder( fCol, fRow, fOneUnit*0.5f, fOneUnit*0.5f, colBorder);
	  HUD_DrawIcon( fCol-18, fRow, _toAllBulletsAll, C_WHITE /*_colHUD*/, fNormValue, FALSE);
      HUD_DrawIcon( fCol, fRow, *ptoCurrentAmmo, C_WHITE /*_colHUD*/,fNormValue, TRUE);
      fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit;
	  if(iCurrentWeapon == WEAPON_TOMMYGUN){
		  FLOAT fMaxValueG = _penWeapons->m_iMaxGrenades;
          FLOAT fValueG = _penWeapons->m_iGrenades;
		  FLOAT fNormValueG = fValueG / fMaxValueG;
		  strValueG.PrintF( "%d", (SLONG)ceil(fValueG));
		  HUD_DrawIcon( fCol-70, fRow, _toAGrenades, C_WHITE /*_colHUD*/ ,fNormValueG, TRUE);
		  HUD_DrawText( fCol-85, fRow, strValueG, NONE, fNormValueG);
	  }
    }
	HUD_DrawText( fCol, fRow, strValue, NONE, fNormValue);
	
	//Reload
	if( _penWeapons->GetMagazinAmmo()){
	  fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit;
	  strValue.PrintF( "%d", (SLONG)ceil(fMagazinValue));
	  //PrepareColorTransitions( C_WHITE, C_WHITE, C_WHITE, C_RED, 0.30f, 0.15f, FALSE);
	  HUD_DrawText(fCol+10, fRow, slash, NONE, fMagazinValue);
	  fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit;
	  HUD_DrawText(fCol+20, fRow, strValue, NONE, fMagazinValue);
	}
	
    fCol -= fAdvUnit+fChrUnit*3/2 -fHalfUnit-35;
	fRow -= fAdvUnit+fChrUnit*3/2 -fHalfUnit-5;
    ...
    fRow = pixBottomBound-fHalfUnit-20;
    fCol = 560 + fHalfUnit;
...
#define BOMB_FIRE_TIME 1.5f
  ...
  fRow = pixBottomBound-fOneUnitS-fAdvUnitS-25;
  //fCol = pixRightBound -fHalfUnitS-500;
  ...
  if( (_tmNow - _penWeapons->m_tmWeaponChangeRequired) < hud_tmWeaponsOnScreen) {
    ...
    fRow = (0.4f*pixBottomBound) - fHalfUnit - 3*fNextUnit;
    fCol = 560.0f-(ctWeapons*fAdvUnit-fHalfUnit)/2.0f;
    // display all available weapons
    for( INDEX ii=WEAPON_NONE+1; ii<WEAPON_LAST; ii++) {
      ...
      if( ptoWantedWeapon == _awiWeapons[i].wi_ptoWeapon) {
        colIcon = 0x19ff1900;
        colBorder = 0x19ff1900;
      }
      // no ammo
      if( _awiWeapons[i].wi_paiAmmo!=NULL && _awiWeapons[i].wi_paiAmmo->ai_iAmmoAmmount==0) {
        HUD_DrawBorder( fCol, fRow, fOneUnit, fOneUnit, 0xb0000000);
        HUD_DrawIcon(   fCol, fRow, *_awiWeapons[i].wi_ptoWeapon, 0xb0000000, 1.0f, FALSE);
      // yes ammo
      } else {
        HUD_DrawBorder( fCol, fRow, fOneUnit, fOneUnit, colBorder);
...
_pDP->FlushRenderingQueue();
 ...
 /* _pDP->PutTexture( &_tohealthBar2, boxProgressBarO2, boxProgressBarChangeO2, C_WHITE|255);
  ...
  _pDP->InitTexture( &_toOxygen);
  _pDP->AddTexture(fHalfUnit, fHalfUnit+695, fHalfUnit+32, fHalfUnit+727, C_WHITE|255);
  _pDP->FlushRenderingQueue();
...
extern void InitHUD(void)
{
  ...
  try {
	...
	_toAllBulletsAll.SetData_t(  CTFILENAME("TexturesMP\\Interface\\BulletsAll.tex"));
    _toHealth.SetData_t(  CTFILENAME("TexturesMP\\Interface\\HealthMax.tex"));
	_toSmallHealth.SetData_t(  CTFILENAME("TexturesMP\\Interface\\HealthSmall.tex"));
	_toMiddleHealth.SetData_t(  CTFILENAME("TexturesMP\\Interface\\HealthMiddle.tex"));
    ...
    _toAPistol.SetData_t(        CTFILENAME("TexturesMP\\Interface\\AmPistolB.tex"));	
	_toASMG.SetData_t(           CTFILENAME("TexturesMP\\Interface\\AmSMG.tex"));
	...
	_toWDoubleColt.SetData_t(      CTFILENAME("TexturesMP\\Interface\\WColt.tex"));
	...
	//initialize draw bar texture 
	_tohealthBar1.SetData_t(  CTFILENAME("TexturesMP\\Interface\\ProgressBarHealth_1.tex"));
	_tohealthBar2.SetData_t(  CTFILENAME("TexturesMP\\Interface\\ProgressBarHealth_2.tex"));
	_toArmorBar.SetData_t(  CTFILENAME("TexturesMP\\Interface\\ProgressBarArmor.tex"));
	_toOxygenBar.SetData_t(  CTFILENAME("TexturesMP\\Interface\\ProgressBarO2.tex"));
	
    // set all textures as constant
    ((CTextureData*)_toHealth .GetData())->Force(TEX_CONSTANT);
	((CTextureData*)_toSmallHealth .GetData())->Force(TEX_CONSTANT);
	((CTextureData*)_toMiddleHealth .GetData())->Force(TEX_CONSTANT);
    ...
    ((CTextureData*)_toAPistol       .GetData())->Force(TEX_CONSTANT);
	((CTextureData*)_toASMG          .GetData())->Force(TEX_CONSTANT);
	...
	((CTextureData*)_toArmorBar      .GetData())->Force(TEX_CONSTANT);
