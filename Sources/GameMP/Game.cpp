...
static void PrintStats( CDrawPort *pdpDrawPort)
{
  ...
  { // determine and clamp size
    ...
    // draw canvas
    pdpDrawPort->Fill(       slDPWidth-1-fMaxWidth, pixJ-ctLines+1, fMaxWidth,   ctLines,   SE_COL_PURPLE_DARK_HV|64);
...
void CGame::GameRedrawView( CDrawPort *pdpDrawPort, ULONG ulFlags)
{
    ...
    && gm_CurrentSplitScreenCfg!=SSC_DEDICATED )
  {
    ...
    if ((ulFlags&GRV_SHOWEXTRAS) && dpMsg.Lock())
    {
      ...
      if (strIndicator!="") {
        ...
        dpMsg.GetWidth()*0.5f, 
        dpMsg.GetHeight()*0.4f, SE_COL_PURPLEGREEN_LT|192);
      }
  ...
  else
  {
    // clear background
    if( pdpDrawPort->Lock()) {
 	    pdpDrawPort->Fill( SE_COL_PURPLE_DARK|CT_OPAQUE);
...
void CGame::LCDRenderClouds1(void)
{
  ...
  if (!IsScriptSoundPlaying(MUSIC_CHANNEL))
  {
    ...
    FLOAT pitch = 1.0f;
    BOOL looping = TRUE;
    PlayScriptSound(MUSIC_CHANNEL, CTFILENAME("Music\\MainMenu\\spellcaster.ogg"), volume, pitch, looping);
  }
  ...
  if (!_bPopup) {
    ...
    _pdp_SE->PutTexture(&_toSamU, box, SE_COL_PURPLE_NEUTRAL|255);
    ...
    _pdp_SE->PutTexture(&_toSamD, box, SE_COL_PURPLE_NEUTRAL|255);
    ...
    _pdp_SE->PutTexture(&_toLeftU, box, SE_COL_PURPLE_NEUTRAL|200);
    ...
    _pdp_SE->PutTexture(&_toLeftD, box, SE_COL_PURPLE_NEUTRAL|200);
    iYU = iYB;
    iYM = iYU + iSize;
    iYB = iYM + iSize;
    iXL = -20;
...
void CGame::LCDRenderCloudsForComp(void)
{
  ...
  _pdp_SE->PutTexture(&_toBcgClouds, _boxScreen_SE, boxBcgClouds1, SE_COL_PURPLE_NEUTRAL|_ulA_SE>>2);
...
void CGame::LCDRenderCompGrid(void)
{
   ...
   _pdp_SE->PutTexture(&_toBcgGrid, _boxScreen_SE, boxBcgGrid, SE_COL_PURPLE_NEUTRAL|_ulA_SE>>1); 
}
...
COLOR CGame::LCDGetColor(COLOR colDefault, const char *strName)
{
  if (!strcmp(strName, "thumbnail border")) {
    ...
    colDefault = SE_COL_PURPLE_DARK_LT|75;
    ...
    colDefault = SE_COL_PURPLE_DARK|255;
  }
