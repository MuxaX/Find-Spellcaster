...
PIX aIconCoords[][2] =
{
  ...
  {0, 0},  // 01: Palenque 01 
  {0, 0},   // 02: Palenque 02 
  {0, 0},   // 03: Teotihuacan 01      
  {0, 0},  // 04: Teotihuacan 02      
  {0, 0},  // 05: Teotihuacan 03      
  {0, 0},  // 06: Ziggurat	 
  {0, 0},  // 07: Atrium		 
  {0, 0},  // 08: Gilgamesh	 
  {0, 0},    // 09: Babel		   
  {0, 0},    // 10: Citadel		 
  {0, 0},   // 11: Land of Damned		     
  {0, 0},    // 12: Cathedral	 
};
...
if (ulLevelMask != 0) {  // Проверяем, что не ноль
    unsigned int index = 0;
    unsigned long temp = ulLevelMask;
    while (temp >>= 1) {  // Сдвигаем вправо, пока не дойдём до 0
        index++;
    }
    
    if (index < sizeof(atoIcons) / sizeof(atoIcons[0])) {  // Проверка границ массива
        pto = &atoIcons[index];
    }
}
  pdp->PutTexture( pto, PIXaabbox2D( PIX2D(0,0), PIX2D(pixdpw,pixdph)), C_WHITE|255);
