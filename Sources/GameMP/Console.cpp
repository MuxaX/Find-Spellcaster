...
void CGame::ConsoleRender(CDrawPort *pdp)
{
  ...
  COLOR colDark  = LCDFadedColor(SE_COL_PURPLE_LIGHT|255);
  ...
  //LCDRenderGrid();
  LCDRenderClouds2();
  dpConsole.DrawLine( 0, pixSizeJ-1, pixSizeI, pixSizeJ-1, LCDFadedColor(SE_COL_PURPLE_NEUTRAL|255));
...
void CGame::ConsolePrintLastLines(CDrawPort *pdp)
{
  ...
  for( INDEX iLine=0; iLine<ctLines; iLine++) {
    ...
    pdp->PutText( strLine, 0, pixCharHeight*(ctLines-iLine-1), SE_COL_PURPLE_LIGHT|255);
  }
}
